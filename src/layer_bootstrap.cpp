...
extern "C" XrResult LAYER_EXPORT
XRAPI_CALL xrNegotiateLoaderApiLayerInterface(const XrNegotiateLoaderInfo *loaderInfo, const char *apiLayerName,
...
// We remove ours form the list if present, and we store the list of *our* extensions that were enabled
    ...
    const auto result = apiLayerInfo->nextInfo->nextCreateApiLayerInstance(&instanceCreateInfo, &newApiLayerCreateInfo,
                                                                           &newInstance);
    if (XR_FAILED(result)) {
