...
OpenXRLayer::OpenXRLayer(PFN_xrGetInstanceProcAddr nextLayerGetInstanceProcAddr) :
    ...
    if (this_layer)
        throw std::runtime_error(
                "Initialization of OpenXR layer " XR_THISLAYER_NAME " failed: Cannot create more than one instance of OpenXRLayer class.");
...
XrResult OpenXRLayer::GetInstanceProcAddr(XrInstance instance,
    ...
    if (!nextLayer_xrGetInstanceProcAddr)
        throw std::runtime_error(
                "Somehow we don't have a pointer to the next layer in the chain's GetInstanceProcAddr function.");
