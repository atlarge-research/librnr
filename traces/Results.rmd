...
```{r val-perf-overhead-gpu-util-all-box, fig.width=5, fig.height=1.6}
d %>%
...
```{r val-perf-overhead-power-draw-all-box, fig.width=5, fig.height=1.6}
d %>%
...
```{r val-perf-vara-gpu-util-all-box, fig.width=3, fig.height=2}
d <- data_logcat_vrapi %>%
...
```{r val-perf-vara-power-draw-all-box, fig.width=3, fig.height=2}
d <- data_batterymanager_companion %>%
  filter(date == 20240228) %>%
  filter(ts > 60) %>%
  filter(ts < 180) %>%
...
d %>%
  ...
  theme_cowplot(15) +
  background_grid() +
  labs(x = "Power draw [W]", y = "Iteration") +
  coord_cartesian(clip="off") +
  theme(plot.margin=margin(0,0,0,0, "cm"), strip.background=element_rect(fill="white"))
```
