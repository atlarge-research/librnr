...
data_net_dev %>%
  ...
  slice(2:n()) %>%
  ungroup() %>%
  ggplot(aes(x = ts_m, y = Mbps_rx, color = factor(bwlimit))) +
  geom_line() +
  theme_cowplot(15) +
  background_grid() +
  xlim(0, NA) +
...
data_net_dev %>%
  ...
  geom_boxplot() +
  stat_summary(fun=mean, geom="point", shape=21, size=2, color="black", fill="white") +
  scale_x_continuous(breaks = seq(0, 100, by = 10), limits=c(0,NA)) +
  theme_cowplot(15) +
  background_grid(major = "x") +
  # xlim(0, NA) +
  labs(x = "Bytes received [Mbps]", y = "Bandwidth limit [Mbps]")
  # facet_grid(cols = vars(game), rows = vars(config))
```
...
data_logcat_vrapi %>%
  ...
  theme_cowplot(15) +
  background_grid() +
  labs(x="Frame rate [fps]", y="Bandwidth Limit [Mbps]") +
  coord_cartesian(clip="off") +
  theme(plot.margin=margin(0,0,0,0, "cm"))
  # facet_grid(cols = vars(config))
```
...
data_host_sys_metrics %>%
  drop_na(bwlimit) %>%
  ggplot(aes(x = cpu.percent, y = factor(bwlimit))) +
  geom_boxplot() +
  xlim(0, 100) +
  labs(x = "CPU utilization [%]", y = "BW Limit [Mbps]    ") +
  theme_cowplot(15) +
...
data_host_gpu_metrics %>%
  drop_na(bwlimit) %>%
  ggplot(aes(x = load * 100, y = factor(bwlimit))) +
  geom_boxplot() +
  xlim(0, 100) +
  labs(x = "GPU utilization [%]", y = "BW Limit [Mbps]    ") +
  theme_cowplot(15) +
...
```{r exp-bwlimit-battery-power-all-box, fig.width=3, fig.height=3}
d <- data_batterymanager_companion %>%
  drop_na(bwlimit) %>%
  filter(i == 2)
...
d %>%
  ...
  theme(legend.position="none", strip.background=element_rect(fill="white")) +
  xlim(0, NA) +
  labs(y = "Bandwidth limit [Mbps]", x = "Power [W]") +
  scale_fill_manual(values=device_colors)
